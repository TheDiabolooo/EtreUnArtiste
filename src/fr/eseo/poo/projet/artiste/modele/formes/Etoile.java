...
public class Etoile extends Forme implements Remplissable{
	...
	public void setNombreBranches(int nbBranches) {
		if(nbBranches >= 3 && nbBranches <= 15) {
			...
			throw new IllegalArgumentException("Une étoile peut avoir entre 3 et 15 branches.");
	...
	public void setLongueurBranche(double longueur) {
		if(longueur >= 0.0 && longueur <= 1.0) {
			...
			throw new IllegalArgumentException("La longueur des branches des branches d'une étoile "
					+ "est réglable (entre 0 et 100% de la longueur du rayon du cercle).");
	}
	...
	public void setAnglePremiereBranche(double angle) {
		if(angle >= (-Math.PI) && angle <= Math.PI) {
			...
			throw new IllegalArgumentException("L’angle formé entre la première branche et "
					+ "l’horizontale est réglable (entre 0 et 2π radians)");
	}
	...
	public boolean contient(Coordonnees coordonnees) {
		...
		for(int i = 0; i<this.getNombreBranches() * 2; i++) {
			...
			Ligne ligneEtoile = getLignes().get(i);
